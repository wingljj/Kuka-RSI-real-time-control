...
private:
    ...
    // ── 指令台账：闭环对象（2026-08-04 真机抖动修复）──────────────────
    // 误差相对它计算，而不是相对实测 RIst。对滞后的实测位姿做比例反馈、经
    // POSCORR（增量积分）施加，等效「积分器 + 对象滞后」闭环：4ms 周期
    // kp=0.1 的等效积分增益 25/s 远超真实伺服滞后（几十 ms）的稳定边界，
    // 真机必然振荡（现场：Z+2mm 疯狂抖动）；模拟器对象无滞后，从未暴露。
    // 相对台账闭环后极点 = 1−kp，与对象动力学无关，无条件稳定；机器人以
    // 自身伺服动态开环跟随指令。RIst 仍用于安全监控（物理跳变、位移、锚点）。
    // 台账只记真正发得出去的量（过了 wire 量化的 d），与 m_accum 同源。
    // 代价：一帧 SEN 在网络上丢失时 KRC 少施加一个增量（≤ 单帧限值），
    // 台账与实际间产生等量常差——它会原样显示在「误差」上（target − RIst
    // 不归零），操作员「归零」即重对齐（resetToActual 同步台账）。
    Pose m_cmd;
    // false = 下一次 step() 时把台账重对齐到当帧实际。使能跟踪的瞬间置位：
    // Idle 期间操作员可能手动移动过机器人，旧台账已失效。
    bool m_cmdSynced = false;
