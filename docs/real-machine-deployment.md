...
五层限值必须**自内向外单调放大**，否则内层永不触发：

| 层 | 位置 | 值 |
|---|---|---|
| 1 单周期增量 | 上位机 `kp × 误差`，夹到 `vmax × cycle` | 0.1 × 10mm/s × 12ms = 0.012 mm |
| 2 锚定位移 | 上位机 `accum_limit_pos_mm` / `_rot_deg`（配置已开到 1000/100） | 首次联机建议临时收紧到 10 mm / 5° |
...
永不会触发，兜住垃圾帧），**PosCorr 限的是累积修正**（真正的天花板）。调整时保持
单调 —— 任何一层比它内侧小，内侧那层就永远不会触发。
...
注意：由「累计命令增量」触发的第 2 层旋转 Fault 无法用界面「归零并复位」清除——
resetToActual 刻意保留累计命令账本，重新使能会立即再次 Fault。只有重启 KRL 程序
（真正的 RSI 会话重启，beginSession 清零账本）才能恢复。
...
4. 上位机参数调最保守：`Kp` 0.1，限速 10 mm/s、2 °/s，累积上限收紧到 10 mm、5°。
...
| RSI 报修正量过大 | 五层梯度反了，或 `Kp` 过高撞上第 4 层 |
| 启动瞬间跳动 | 首帧未同步目标；检查第 6 步「误差全为 0」是否被跳过 |
| 误差曲线持续振荡 | `Kp` 过高，减半再试 |
| 机器人方向与预期相反 | 基坐标系不是你以为的那个（见 §5 第 2 步） |
| 故障 `displacement from session anchor ... exceeds limit` | 第 2 层触发，正常保护。归零到当前位姿可继续，但预算已耗尽，真正复位要重启 KRL |
